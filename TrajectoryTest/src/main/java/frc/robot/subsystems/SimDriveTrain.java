...
public class SimDriveTrain extends SubsystemBase implements Constants {
 
  private static final int kEncoderResolution = -4096;

  private final PWMSparkMax m_leftLeader = new PWMSparkMax(FRONT_LEFT);
  private final PWMSparkMax m_leftFollower = new PWMSparkMax(BACK_LEFT);
  private final PWMSparkMax m_rightLeader = new PWMSparkMax(FRONT_RIGHT);
  private final PWMSparkMax m_rightFollower = new PWMSparkMax(BACK_RIGHT);
  ...
  private final DifferentialDriveKinematics m_kinematics = new DifferentialDriveKinematics(trackWidth);
  ...
  private final DifferentialDrivetrainSim m_drivetrainSimulator = new DifferentialDrivetrainSim(m_drivetrainSystem,
      DCMotor.getCIM(2), 8, trackWidth, wheelRadius, null);
  ...
  public SimDriveTrain() {
    ...
    // resolution.
    m_leftEncoder.setDistancePerPulse(2 * Math.PI * wheelRadius / kEncoderResolution);
    m_rightEncoder.setDistancePerPulse(2 * Math.PI * wheelRadius / kEncoderResolution);
